    ...
    for (const auto& itkeys : keys_dedup) {
        ...
        if (itkeys.first==0) {
            ...
            if (itkeys.first==last_key) {
                ...
                //set the worlvolume in the recoGeoSvc
                m_recoGeoSvc->setWorldVolume(world);
            }
...
std::vector<float> RecoGeoConverterTool::orderRValues(std::vector<std::pair<float,float>>& old) const
{
...
bool RecoGeoConverterTool::sortFloatPairs(std::pair<float,float> ap, std::pair<float,float> bp)
{
...
bool RecoGeoConverterTool::sortZvolumes(const std::pair<std::shared_ptr<const Reco::Volume>, Alg::Point3D>& a,const std::pair<std::shared_ptr<const Reco::Volume>, Alg::Point3D>& b)
{
    return (a.second.Z()<b.second.Z());
}

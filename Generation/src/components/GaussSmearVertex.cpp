...
StatusCode GaussSmearVertex::initialize( ) {
  ...
  info() << "Smearing of interaction point with normal distribution "
         << " in x, y and z " << endmsg;
  info() << infoMsg << endmsg;
  info() << " with " << m_xsig / Gaudi::Units::mm
         << " mm  standard deviation in x "
         << m_ysig / Gaudi::Units::mm << " mm in y and "
         << m_zmin / Gaudi::Units::mm << " mm in z." << endmsg;

  if ( ! sc.isSuccess() )
    return Error( "Could not initialize normal random number generator" ) ;
...
StatusCode GaussSmearVertex::smearVertex( HepMC::GenEvent& theEvent ) {
  ...
  dx = m_xmin + m_gaussDistX( );
  dy = m_ymin + m_gaussDistY( );
  dz = m_zmin + m_gaussDistZ( );

...
HepEVTReader::HepEVTReader(const std::string& name, ISvcLocator* svcLoc):
  ...
  m_filename() {
  std::cout << "nalipourTest: HepEVTReader constructor" << std::endl;
  ...
  // declareOutput("hepevt", m_hepmchandle);
}
...
StatusCode HepEVTReader::initialize()
{
  ...
  std::cout << "nalipourTest: m_filename=" << m_filename << std::endl; 
  ...
  m_input >> NHEP;
  info() << "nalipourTest: NHEP=" << NHEP << endmsg;

...
// To be used with the Output of the BFieldWriter.
...
/// @note The default parameters are set for the FCC field
void printBField(std::string inFile,
                 std::string treeName = "bField",
                 std::string outFile = "printBField_FCC.root",
                 float rmin = 0.,
                 float rmax = 20.,
                 float zmin = -30.,
                 float zmax = 30.,
  ...
  TProfile2D* bField_rz = new TProfile2D("BField_rz", "Magnetic Field", nBins, zmin, zmax, nBins * 0.5, 0., rmax);
  bField_rz->GetXaxis()->SetTitle("z [m]");
  bField_rz->GetYaxis()->SetTitle("r [m]");
  TProfile2D* bField_xy = new TProfile2D("BField_xy", "Magnetic Field", nBins, rmin, rmax, nBins, rmin, rmax);
  bField_xy->GetXaxis()->SetTitle("x [m]");
  bField_xy->GetYaxis()->SetTitle("y [m]");
  TProfile2D* bField_yz = new TProfile2D("BField_yz", "Magnetic Field", nBins, zmin, zmax, nBins, rmin, rmax);
  bField_yz->GetXaxis()->SetTitle("z [m]");
  bField_yz->GetYaxis()->SetTitle("y [m]");
  TProfile2D* bField_xz = new TProfile2D("BField_xz", "Magnetic Field", nBins, zmin, zmax, nBins, rmin, rmax);

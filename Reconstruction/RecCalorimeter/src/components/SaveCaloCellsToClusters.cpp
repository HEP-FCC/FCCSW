#include "SaveCaloCellsToClusters.h"
...
DECLARE_ALGORITHM_FACTORY(SaveCaloCellsToClusters)

SaveCaloCellsToClusters::SaveCaloCellsToClusters(const std::string& name, ISvcLocator* svcLoc)
...
SaveCaloCellsToClusters::~SaveCaloCellsToClusters()
{
}

StatusCode SaveCaloCellsToClusters::initialize() {
...
StatusCode SaveCaloCellsToClusters::execute() {
  ...
  for (const auto& ecells : *calocells) {
    ...
    if (r_cell_size_half<0) {
      if ((*m_decoder)[m_activeFieldName]<m_numLayers) {
	(*m_decoder)[m_activeFieldName]=(*m_decoder)[m_activeFieldName]+1;
	uint64_t cellID_next = m_decoder->getValue();
    ...
    debug() << "cellID " << ecells.Core().Cellid <<" energy " << ecells.Core().Energy << " decoder: all fields " << m_decoder->valueString() << " r " << r_cell << " eta " <<  m_segmentation->eta(ecells.Core().Cellid) << " phi " <<  m_segmentation->phi(ecells.Core().Cellid)<< endmsg;
  }
...
StatusCode SaveCaloCellsToClusters::finalize() {  

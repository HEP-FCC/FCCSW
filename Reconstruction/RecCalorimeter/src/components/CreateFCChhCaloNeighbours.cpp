...
CreateFCChhCaloNeighbours::CreateFCChhCaloNeighbours(const std::string& aName, ISvcLocator* aSL) : base_class(aName, aSL) {
...
StatusCode CreateFCChhCaloNeighbours::initialize() {
  ...
  for(uint iSys = 0; iSys < m_readoutNames.size(); iSys++) {
    ...
    if (m_geoSvc->lcdd()->readouts().find(m_readoutNames[iSys]) == m_geoSvc->lcdd()->readouts().end()) {
      error() << "Readout <<" << m_readoutNames[iSys] << ">> does not exist." << endmsg;
      return StatusCode::FAILURE;
    }
    // get PhiEta segmentation
    DD4hep::DDSegmentation::GridPhiEta* segmentation;
    segmentation = dynamic_cast<DD4hep::DDSegmentation::GridPhiEta*>(m_geoSvc->lcdd()->readout(m_readoutNames[iSys]).segmentation().segmentation());
    ...
    auto decoder = m_geoSvc->lcdd()->readout(m_readoutNames[iSys]).idSpec().decoder();

    // Loop over all cells in the calorimeter and retrieve existing cellIDs
    // Loop over active layers
    std::vector<std::pair<int, int>> extrema;
    extrema.push_back(std::make_pair(0, m_activeVolumesNumbers[iSys]-1));
    extrema.push_back(std::make_pair(0, 0));
    extrema.push_back(std::make_pair(0, 0));
    for (unsigned int ilayer = 0; ilayer < m_activeVolumesNumbers[iSys]; ilayer++) {
      // Get VolumeID
      (*decoder)[m_fieldNames[iSys]] = m_fieldValues[iSys];
      (*decoder)[m_activeFieldNames[iSys]] = ilayer;
      (*decoder)["eta"] = 0;
      (*decoder)["phi"] = 0;
      ...
      for (unsigned int iphi = 0; iphi < numCells[0]; iphi++) {
        for (unsigned int ieta = numCells[2]; ieta < numCells[1]; ieta++) {
          ...
          map.insert(std::pair<uint64_t, std::vector<uint64_t>>(cellId, det::utils::neighbours(*decoder, {"layer","phi","eta"}, extrema, cellId, false)));
//rootmap.Add(TObject(cellId), TObject(det::utils::neighbours(*decoder, {"layer","phi","eta"}, extrema, cellId, true)));
        }
      }
	...
	if (counterL[iCount] != 0) {
	  info() << "layer " << ilayer << " : " << counterL[iCount] << " cells have " << iCount << " neighbours" << endmsg;
	}
      }
    }
...
StatusCode CreateFCChhCaloNeighbours::finalize() {

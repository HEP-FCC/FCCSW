...
NestedVolumesCaloTool::~NestedVolumesCaloTool() {}
...
StatusCode NestedVolumesCaloTool::finalize() {
...
StatusCode NestedVolumesCaloTool::prepareEmptyCells(std::unordered_map<uint64_t, double>& aCells) {
  ...
  for(uint it = 0; it < m_activeVolumeName.size(); it++) {
    // names of volumes (m_activeVolumeName) not needed anymore, only corresponding bitfield names are used (m_activeFieldName)
    numVolumesMap.push_back(std::pair<std::string, int>(m_activeFieldName[it], numVolumes[it]));
  }
  std::sort(numVolumesMap.begin(),numVolumesMap.end(),
    [](std::pair<std::string, int> vol1, std::pair<std::string, int> vol2)
    { return vol1.second < vol2.second; });
  // now recompute how many volumes exist within the larger volume
  for(uint it = numVolumesMap.size() - 1; it > 0; it--) {
  ...
  uint numVolTypes = numVolumes.size();
  currentVol.assign(numVolTypes,0);
  uint index = 0;
  do {
    index = 0;
    for(uint it = 0; it < numVolTypes; it++) {

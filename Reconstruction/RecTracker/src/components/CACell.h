...
public:
  ...
  CACell(const HitDoublets* doublets, int doubletId, const unsigned int cellId, const int innerHitId,
  ...
  unsigned int getCellId() const { return theCellId; }
  ...
  float getOuterZ() const { return theOuterZ; }

  float getInnerR() const { return theInnerR; }

  float getOuterR() const { return theOuterR; }
  ...
  unsigned int getInnerHitId() const { return theInnerHitId; }

  unsigned int getOuterHitId() const { return theOuterHitId; }
  ...
  bool areAlignedRZ(const CACell* otherCell, const float ptmin, const float thetaCut) const {
    ...
    float tan_12_13_half_mul_distance_13_squared =
        fabs(z1 * (theInnerR - theOuterR) + theInnerZ * (theOuterR - r1) + theOuterZ * (r1 - theInnerR));
    return tan_12_13_half_mul_distance_13_squared * ptmin <= thetaCut * distance_13_squared;
  }

  void tagAsOuterNeighbor(CACell* otherCell) { theOuterNeighbors.push_back(otherCell); }

  void tagAsInnerNeighbor(CACell* otherCell) { theInnerNeighbors.push_back(otherCell); }

  bool haveSimilarCurvature(const CACell* otherCell, const float region_origin_x, const float region_origin_y,
  ...
  unsigned int getCAState() const { return theCAState; }
  ...
  void findNtuplets(std::vector<CAntuplet>& foundNtuplets, CAntuplet& tmpNtuplet,

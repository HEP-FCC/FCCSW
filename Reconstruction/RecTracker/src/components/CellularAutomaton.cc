...
int CellularAutomaton<numberOfLayers>::test() {
  return 3;
}
...
void CellularAutomaton<numberOfLayers>::createAndConnectCells(std::vector<const HitDoublets*> doublets,
  ...
  for (unsigned int i = 0; i < numberOfDoublets; ++i) {
    ...
    theFoundCellsPerLayer[layerPairId].emplace_back(doubletLayerPairId, i, cellId, doubletLayerPairId->innerHitId(i),
  ...
  for (layerPairId = 1; layerPairId < numberOfLayerPairs; ++layerPairId) {
    ...
    for (unsigned int i = 0; i < numberOfDoublets; ++i) {
      theFoundCellsPerLayer[layerPairId].emplace_back(
          doublets[layerPairId], i, cellId, doubletLayerPairId->innerHitId(i), doubletLayerPairId->outerHitId(i));
      isOuterHitOfCell[outerLayerId][doubletLayerPairId->outerHitId(i)].push_back(
          &(theFoundCellsPerLayer[layerPairId][i]));
      cellId++;
...
template <unsigned int numberOfLayers>
void CellularAutomaton<numberOfLayers>::findNtuplets(std::vector<CACell::CAntuplet>& foundNtuplets,

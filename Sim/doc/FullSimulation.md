...
Additionally, the simulation service takes the random number generator seeds from Gaudi service `RndmGenSvc`. If user wants to set the simulation seeds manually, flag `rndmFromGaudi` needs to be set to `false`.

...
DECLARE_COMPONENT(SimG4BFieldTool)

SimG4BFieldTool::SimG4BFieldTool(const std::string& type, const std::string& name, const IInterface* parent)
    : GaudiTool(type, name, parent), m_field(nullptr) {
  declareInterface<ISimG4MagneticFieldTool>(this);
}

SimG4BFieldTool::~SimG4BFieldTool() {
  if (nullptr != m_field) delete m_field;
}

StatusCode SimG4BFieldTool::initialize() {
...
StatusCode SimG4BFieldTool::finalize() {
  StatusCode sc = GaudiTool::finalize();
  return sc;
}

const G4MagneticField* SimG4BFieldTool::field() const { return m_field; }

G4MagIntegratorStepper* SimG4BFieldTool::stepper(const std::string& name, G4MagneticField* field) const {

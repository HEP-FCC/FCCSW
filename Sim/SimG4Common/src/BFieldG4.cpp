...
BFieldG4::BFieldG4(ServiceHandle<IBFieldSvc> bFieldSvc)
    : G4MagneticField(), m_bFieldSvc(bFieldSvc), m_fieldCell(m_bFieldSvc->getFieldCell(fcc::Vector3D{0., 0., 0.})) {}

void BFieldG4::GetFieldValue(const double* point, double* bField) const {
  // translate the point
 fcc::Vector3D position(point);
  ...
  fcc::Vector3D field = (m_fieldCell.getField(position)/Acts::units::_T)*CLHEP::tesla;
  // tranlate the output

...
public:
   ...
   *  Checks on the state of the Geant4 (proper initialization of the geometry, physics list, kernel and also if no other simulation has been already running).
   *  @warning This method should be called \b after calling SetUserInitialization(G4VUserDetectorConstruction*) and SetUserInitialization(G4VUserPhysicsList*).
   *  @returns the status code
   */
  StatusCode start();
  /** Processing of the event.
   *  It substitutes the G4RunManager::ProcessOneEvent(int) method (excluding the generation part).
   *  It checks if the previous event has been fully processed (including a call to terminateEvent()) and begins its simulation.

...
unsigned long Reco::ReadoutSegmentation1D::bin(const Alg::Point2D& locpos) const
{
...
const std::vector<unsigned long> Reco::ReadoutSegmentation1D::compatibleBins(const Alg::Point2D& locpos) const
{
    ...
    long binsigned    = bin;
    if ((binsigned-1) >= 0)     bins.push_back(bin-1);
                                bins.push_back(bin);
...
float Reco::ReadoutSegmentation1D::binwidth(const Alg::Point2D& locpos, size_t) const
{

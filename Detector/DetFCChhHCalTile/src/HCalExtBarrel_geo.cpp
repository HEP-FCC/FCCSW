...
createHCal(DD4hep::Geometry::LCDD& lcdd, xml_h xmlElement, DD4hep::Geometry::SensitiveDetector sensDet) {
  ...
  Volume envelopeVolumePos("envelopeVolumePos", envelopeShapePos, lcdd.air());
  envelopeVolumePos.setVisAttributes(lcdd, dimensions.visStr()); 
  Volume envelopeVolumeNeg("envelopeVolumeNeg", envelopeShapeNeg, lcdd.air());
  ...
  DD4hep::Geometry::Position offsetFace1front(0, 0, -dzDetector2);
  DD4hep::Geometry::Position offsetFace1back(0, 0, dzDetector2);
  ...
  DD4hep::Geometry::Position offsetFace2front(0, 0, dzDetector1);
  DD4hep::Geometry::Position offsetFace2back(0, 0, -dzDetector1);

  PlacedVolume placedFacePlate1front = envelopeVolumePos.placeVolume(facePlateVol1, offsetFace1front);
  facePlate1.setPlacement(placedFacePlate1front);
  PlacedVolume placedFacePlate1back = envelopeVolumeNeg.placeVolume(facePlateVol1, offsetFace1back);
  facePlate1.setPlacement(placedFacePlate1back);
  PlacedVolume placedFacePlate2front = envelopeVolumePos.placeVolume(facePlateVol2, offsetFace2front);
  facePlate2.setPlacement(placedFacePlate2front);
  PlacedVolume placedFacePlate2back = envelopeVolumeNeg.placeVolume(facePlateVol2, offsetFace2back);
  ...
  DD4hep::Geometry::Position posOffsetFront(0, 0, dzDetector1 + dzDetector2 - dZEndPlate);
  DD4hep::Geometry::Position posOffsetBack(0, 0, -dzDetector1 - dzDetector2 + dZEndPlate);
  PlacedVolume placedEndPlatePosFront = envelopeVolumePos.placeVolume(endPlateVol2, posOffsetFront);
  endPlatePos.setPlacement(placedEndPlatePosFront);
  PlacedVolume placedEndPlatePosBack = envelopeVolumeNeg.placeVolume(endPlateVol2, posOffsetBack);
  endPlatePos.setPlacement(placedEndPlatePosBack);

  DetElement endPlateNeg(hCal, "endPlateNeg", 1);
  DD4hep::Geometry::Position negOffsetFront(0, 0, -dzDetector2 - dzDetector1 + dZEndPlate);
  DD4hep::Geometry::Position negOffsetBack(0, 0, dzDetector2 + dzDetector1 - dZEndPlate);
  PlacedVolume placedEndPlateNegFront = envelopeVolumePos.placeVolume(endPlateVol1, negOffsetFront);
  endPlateNeg.setPlacement(placedEndPlateNegFront);
  PlacedVolume placedEndPlateNegBack = envelopeVolumeNeg.placeVolume(endPlateVol1, negOffsetBack);
  ...
  for (unsigned int idxPhi = 0; idxPhi < numSequencesPhi; ++idxPhi) {
    ...
    DD4hep::Geometry::Position moduleOffset1 (xPosModule1,  yPosModule1, ( -dzDetector2 + 2*dZEndPlate + space));
    DD4hep::Geometry::Position moduleOffset1_2 (xPosModule1,  yPosModule1, ( dzDetector2 - 2*dZEndPlate - space));
    DD4hep::Geometry::Position supportOffset1(xPosSupport1, yPosSupport1, ( -dzDetector2 + 2*dZEndPlate + space));
    DD4hep::Geometry::Position supportOffset1_2(xPosSupport1, yPosSupport1, ( dzDetector2 - 2*dZEndPlate - space));
    DD4hep::Geometry::Position moduleOffset2 (xPosModule2,  yPosModule2, ( dzDetector1 - 2*dZEndPlate - space));
    DD4hep::Geometry::Position moduleOffset2_2 (xPosModule2,  yPosModule2, ( -dzDetector1 + 2*dZEndPlate + space));
    DD4hep::Geometry::Position supportOffset2(xPosSupport2, yPosSupport2, ( dzDetector1 - 2*dZEndPlate - space));
    DD4hep::Geometry::Position supportOffset2_2(xPosSupport2, yPosSupport2, ( -dzDetector1 + 2*dZEndPlate + space));
    ...
    modules1.push_back( envelopeVolumePos.placeVolume(moduleVolume1, trans1) );
    modules1.back().addPhysVolID("module1_", idxPhi);
    modules1.push_back( envelopeVolumeNeg.placeVolume(moduleVolume1, trans1B) );
    modules1.back().addPhysVolID("module1_", numSequencesPhi + idxPhi);

    modules2.push_back( envelopeVolumePos.placeVolume(moduleVolume2, trans2) );
    modules2.back().addPhysVolID("module2_", idxPhi);
    modules2.push_back( envelopeVolumeNeg.placeVolume(moduleVolume2, trans2B) );
    modules2.back().addPhysVolID("module2_", numSequencesPhi + idxPhi);

    supports1.push_back( envelopeVolumePos.placeVolume(steelSupportVolume1, trans1S) );
    supports1.push_back( envelopeVolumeNeg.placeVolume(steelSupportVolume1, trans1BS) );
    supports2.push_back( envelopeVolumePos.placeVolume(steelSupportVolume2, trans2S) );
    supports2.push_back( envelopeVolumeNeg.placeVolume(steelSupportVolume2, trans2BS) );
  }
  ...
  PlacedVolume placedPosHCal = motherVol.placeVolume(envelopeVolumePos, DD4hep::Geometry::Position(0, 0, (extBarrelOffset1 + dimensions.dz())));
  PlacedVolume placedNegHCal = motherVol.placeVolume(envelopeVolumeNeg, DD4hep::Geometry::Position(0, 0, -(extBarrelOffset1 + dimensions.dz())));

  placedPosHCal.addPhysVolID("system", hCal.id());
  placedNegHCal.addPhysVolID("system", hCal.id());
  
  hCal.setPlacement(placedPosHCal);
  hCal.setPlacement(placedNegHCal);
  return hCal;
}
}  // namespace hcal

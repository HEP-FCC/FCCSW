...
static DD4hep::Geometry::Ref_t createSimpleBoxes(DD4hep::Geometry::LCDD& aLcdd,
  ...
  std::cout << "cell size: "<<cell_dim.x()<<"\t"<<cell_dim.y()<<"\t"<<cell_dim.z()<<std::endl;
  DD4hep::Geometry::Material cell_mat (aLcdd.material(cell_det.materialStr()));
// we assume same no of cells each dir (x,y,z)
  unsigned int numCells = dim.x()/cell_dim.x();
  double offset = -1.*dim.x() + cell_dim.x();
  std::cout << "number of cells: "<<numCells<<std::endl;
  std::cout<<"offset: "<<offset<<std::endl;
  DD4hep::Geometry::Volume cell_vol("cell_vol",DD4hep::Geometry::Box(cell_dim.x(),cell_dim.y(),cell_dim.z()), cell_mat);
  ...
  for(uint ix=0;ix<numCells; ix++) {
    ...
    DD4hep::Geometry::Transform3D transformcell(DD4hep::Geometry::Rotation3D(
        DD4hep::Geometry::RotationZYX(0,0,0)), DD4hep::Geometry::Position(offset+ix*2*cell_dim.x(), 0,0 ));
    DD4hep::Geometry::PlacedVolume cell_physvol = row_vol.placeVolume(cell_vol,transformcell);
    cell_physvol.addPhysVolID("modulex", ix);
    // DD4hep::Geometry::DetElement cell_det(cellName,ix);
    // cell_det.setPlacement(cell_physvol);
  }
  for(uint iy=0;iy<numCells; iy++) {
    auto cellName = std::string("row" ) + DD4hep::XML::_toString(iy, "y%d");
    DD4hep::Geometry::Transform3D transformrow(DD4hep::Geometry::Rotation3D(
        DD4hep::Geometry::RotationZYX(0,0,0)), DD4hep::Geometry::Position(0,offset+iy*2*cell_dim.y(),0 ));
    DD4hep::Geometry::PlacedVolume row_physvol = layer_vol.placeVolume(row_vol,transformrow);
    row_physvol.addPhysVolID("moduley", iy);
    // DD4hep::Geometry::DetElement row_det(cellName,iy);
    // row_det.setPlacement(row_physvol);
  }
  for(uint iz=0;iz<numCells; iz++) {
    auto cellName = std::string("layer" ) + DD4hep::XML::_toString(iz, "z%d");
    DD4hep::Geometry::Transform3D transformlayer(DD4hep::Geometry::Rotation3D(
        DD4hep::Geometry::RotationZYX(0,0,0)), DD4hep::Geometry::Position(0,0,offset+iz*2*cell_dim.z() ));
    ...
    // DD4hep::Geometry::DetElement layer_det(cellName,iz);
    // layer_det.setPlacement(layer_physvol);
  }

...
static DD4hep::Geometry::Ref_t createECal(DD4hep::Geometry::LCDD& lcdd, xml_h xmlElement,
  ...
  if (cryo_thickness > 0) {
    ...
    DD4hep::Geometry::Tube bathShape(cryo_dims.rmin() + cryo_thickness, cryo_dims.rmax() - cryo_thickness,
                                     cryo_dims.dz() - cryo_thickness);
    lLog << MSG::DEBUG << "ECAL " << active_mat << " bath: rmin " << cryo_dims.rmin() + cryo_thickness << " rmax "
         << cryo_dims.rmax() - cryo_thickness << endmsg;
  ...
  lLog << MSG::DEBUG << "ECAL actual calorimeter: rmin " << calo_dims.rmin() << " rmax " << calo_dims.rmin() + calo_tck
       << endmsg;
  ...
  lLog << MSG::DEBUG << "Passive material:  steel = 2x " << steel_thck << ", glue = 2x " << glue_thck
       << ", lead = " << pb_thck << endmsg;
  ...
  for (int i = 0; i < active_samples; i++) {
    ...
    DD4hep::Geometry::Tube glueShape1(layer_r - passive_tck + steel_thck,
                                      layer_r - passive_tck + steel_thck + glue_thck, calo_dims.dz());
    DD4hep::Geometry::Tube passiveShape(layer_r - passive_tck + steel_thck + glue_thck,
                                        layer_r - steel_thck - glue_thck, calo_dims.dz());
    ...
    lLog << MSG::DEBUG << "layer borders: " << layer_r - passive_tck << "\t" << layer_r - passive_tck + steel_thck
         << "\t" << layer_r - passive_tck + steel_thck + glue_thck << "\t" << layer_r - steel_thck - glue_thck << "\t"
         << layer_r - steel_thck << "\t" << layer_r << "\t" << layer_r + active_tck << endmsg;
    if (cryo_thickness > 0) {
